...
use crate::services::downloader::dependencies::mirror::{
    route_asset_object_urls, route_assets_index_urls,
};
...
#[tauri::command]
pub async fn run_download_benchmark<R: Runtime>(
    app: AppHandle<R>,
) -> Result<DownloadBenchmarkReport, String> {
    ...
    let temp_root = prepare_benchmark_temp_dir(&app)
        .await
        .map_err(|error| error.to_string())?;

    let assets_sources = resolve_assets_sources(&client, &download_settings)
        .await
        .unwrap_or_default();
    let java_sources = resolve_java_sources(&client).await.unwrap_or_default();
    let loader_sources = resolve_loader_sources(&app, &download_settings)
        .await
        .unwrap_or_default();

    let assets = benchmark_sources(
        &client,
        &temp_root,
        "assets",
        &assets_sources,
...
async fn benchmark_sources(
    ...
    sources: &[NamedSource],
    dl_settings: &DownloadSettings,
) -> Vec<DownloadBenchmarkResult> {
    let mut results = Vec::with_capacity(sources.len());
    for (index, source) in sources.iter().enumerate() {
        results
            .push(benchmark_source(client, temp_root, category, index, source, dl_settings).await);
    }
    results
}
...
async fn benchmark_source(
    ...
    let base_name = format!(
        ...
        category,
        index,
        sanitize_file_component(&source.source_id)
    );
    let single_path = temp_root.join(format!("{}-single.bin", base_name));
    let concurrent_root = temp_root.join(format!("{}-concurrent", base_name));

    let range_supported = detect_range_support(client, &source.url)
        .await
        .unwrap_or(false);
    let single_metrics =
        probe_partial_download(client, &source.url, &single_path, dl_settings).await;
    let concurrent_metrics = if single_metrics.is_ok() {
        probe_concurrent_downloads(client, &source.url, &concurrent_root, dl_settings)
            .await
            .ok()
    } else {
        None
...
async fn probe_partial_download(
    client: &reqwest::Client,
    url: &str,
    temp_path: &Path,
    dl_settings: &DownloadSettings,
) -> Result<DownloadProbeMetrics, String> {
...
async fn probe_concurrent_downloads(
    client: &reqwest::Client,
    url: &str,
    temp_dir: &Path,
    dl_settings: &DownloadSettings,
...
async fn resolve_assets_sources(
    client: &reqwest::Client,
    dl_settings: &DownloadSettings,
) -> Result<Vec<NamedSource>, String> {
    let version_manifest_text =
        fetch_text_from_candidates(client, &build_version_manifest_urls(dl_settings)).await?;
    let manifest_json: Value =
        serde_json::from_str(&version_manifest_text).map_err(|error| error.to_string())?;
    ...
    let asset_index_text = fetch_text_from_candidates(
        client,
        &route_assets_index_urls(asset_index_url, dl_settings),
    )
    .await?;
    let asset_index_json: Value =
        serde_json::from_str(&asset_index_text).map_err(|error| error.to_string())?;
    let (hash, _) = select_asset_sample(&asset_index_json)
        .ok_or_else(|| "unable to find a 200KB asset sample".to_string())?;
    let prefix = &hash[0..2];
...
async fn resolve_java_sources(client: &reqwest::Client) -> Result<Vec<NamedSource>, String> {
    ...
    for provider in ["adoptium", "zulu", "tuna", "aks"] {
        if let Some((source_id, source_name, url)) =
            resolve_java_download_url(client, provider).await?
        {
            push_named_source(
                &mut sources,
                &mut seen,
...
async fn resolve_java_download_url(
    client: &reqwest::Client,
    provider: &str,
    ...
    match provider {
        ...
        "zulu" => {
            ...
            Ok(Some((
                "zulu".to_string(),
                "Java / Azul Zulu".to_string(),
                url.to_string(),
            )))
        }
        "tuna" => {
            let tuna_os = match os {
                "mac" => "mac",
                _ => os,
            };
            let tuna_arch = match arch {
                "x64" => "x64",
                "aarch64" => "aarch64",
                _ => "x64",
            };
            let archive_pattern = if ext == "zip" {
                r#"href="([^"]+\.zip)""#
            } else {
                r#"href="([^"]+\.tar\.gz)""#
            };
            let regex = Regex::new(archive_pattern).map_err(|error| error.to_string())?;
            for package_type in ["jre", "jdk"] {
                let index_url = format!(
                    "https://mirrors.tuna.tsinghua.edu.cn/Adoptium/{}/{}/{}/{}/",
                    DOWNLOAD_TEST_JAVA_VERSION, package_type, tuna_arch, tuna_os
                );
                let html = client
                    .get(&index_url)
                    .send()
                    ...
                    })
                {
                    return Ok(Some((
                        "tuna".to_string(),
                        "Java / Tsinghua TUNA".to_string(),
                        format!("{}{}", index_url, file_name),
                    )));
                }
            }

            Err("missing TUNA Adoptium download url".to_string())
        }
        "aks" => {
            let aks_os = match os {
                "mac" => "macOS",
            ...
            Ok(Some((
                ...
                format!(
                    ...
                    DOWNLOAD_TEST_JAVA_VERSION, aks_os, aks_arch, ext
                ),
            )))
        }
        _ => Ok(None),
...
async fn fetch_first_non_empty_json_array(
    client: &reqwest::Client,
    urls: &[String],
) -> Result<Value, String> {
    for url in urls {
...
async fn resolve_loader_sources<R: Runtime>(
    app: &AppHandle<R>,
    dl_settings: &DownloadSettings,
) -> Result<Vec<NamedSource>, String> {
    let mut sources = Vec::new();
...
async fn resolve_latest_loader_version<R: Runtime>(
    app: &AppHandle<R>,
    loader_type: &str,
) -> Result<Option<String>, String> {
    let versions =
        LoaderMetadataService::fetch_loader_versions(app, loader_type, DOWNLOAD_TEST_GAME_VERSION)
            .await
            .map_err(|error| error.to_string())?;
...
fn build_forge_loader_sources(dl_settings: &DownloadSettings, version: &str) -> Vec<NamedSource> {
    let artifact_path = format!(
        ...
        DOWNLOAD_TEST_GAME_VERSION, version
    );
    let mut sources = Vec::new();
    let mut seen = HashSet::new();
...
fn build_neoforge_loader_sources(
    dl_settings: &DownloadSettings,
    version: &str,
) -> Vec<NamedSource> {
    let artifact_path = format!(
