...
pub async fn import_panorama_from_pack<R: Runtime>(
    ...
    if !source.exists() {
        return Err("选中的材质包不存在".to_string());
    }

    // Derive a clean pack name from the file/folder name
    let raw_name = source
        .file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("UnknownPack")
        .to_string();
    // Sanitize: replace characters that are invalid in filenames or the regex pattern
    let pack_name = raw_name
        .chars()
        .map(|c| {
    ...
    let internal_prefix = "assets/minecraft/textures/gui/title/background/";
    let mut extracted_count: usize = 0;

    if source.is_dir() {
        // ── Handle extracted folder ──
        for filename in &panorama_filenames {
            let src_file = source.join(internal_prefix).join(filename);
            if !src_file.exists() {
                continue;
            }
            ...
            let dest = target_dir.join(format!("{}_resourcepack_{}", pack_name, filename));
        ...
        let ext = source
            ...
            .to_ascii_lowercase();
        if ext != "zip" {
            return Err("不支持的材质包格式，请选择 .zip 文件或已解压的文件夹".to_string());
        }
        ...
        for filename in &panorama_filenames {
            // Try both with and without leading slash variations
            let zip_path = format!("{}{}", internal_prefix, filename);
            let mut zip_file = match archive.by_name(&zip_path) {
                Ok(f) => f,
                Err(_) => continue,
            };
            ...
            zip_file
                .read_to_end(&mut buf)
                .map_err(|e| format!("读取 ZIP 中的 {filename} 失败: {e}"))?;
    ...
    if extracted_count == 0 {
        // Clean up the empty directory we may have created
        let _ = fs::remove_dir(&target_dir);
        return Err(
            "在材质包中未找到全景图文件。请确认材质包包含 assets/minecraft/textures/gui/title/background/panorama_0..5.png"
                .to_string(),
        );
    }
    ...
    if extracted_count < 6 {
        return Err(format!(
            "仅找到 {extracted_count}/6 张全景图。天空盒需要完整的 6 张 panorama_0..5.png 才能正常显示。"
        ));
    }
