    ...
    pub fn resolve<R: Runtime>(
        ...
        let instance_runtime = runtime_service::get_instance_runtime(&instance_dir).unwrap_or_else(|_| {
            crate::domain::runtime::RuntimeConfig {
                use_global_java: true, use_global_memory: true,
                java_path: "".to_string(), max_memory: 4096, min_memory: 1024, jvm_args: "".to_string(),
            }
        });

        let mut java_path = if instance_runtime.use_global_java || instance_runtime.java_path.is_empty() {
            global_java.java_path.clone()
        } else { instance_runtime.java_path.clone() };
        ...
        let mut custom_jvm_args: Vec<String> = global_java.jvm_args.split_whitespace().map(|s| s.to_string()).collect();

        if !instance_runtime.use_global_java && !instance_runtime.jvm_args.is_empty() {
            let extra_args: Vec<String> = instance_runtime.jvm_args.split_whitespace().map(|s| s.to_string()).collect();
            custom_jvm_args.extend(extra_args);
        }

        ResolvedLaunchConfig {
            java_path, min_memory, max_memory, resolution_width, resolution_height,
            fullscreen: global_game.fullscreen, custom_jvm_args,
        }
    }
}