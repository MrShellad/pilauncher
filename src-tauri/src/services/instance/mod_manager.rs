...
impl ModManagerService {
    ...
    // ================= 1. 读取并解析 Mods =================
    pub fn get_mods<R: Runtime>(app: &AppHandle<R>, instance_id: &str) -> Result<Vec<ModMetadata>, String> {
                ...
                if path.is_file() {
                    ...
                    if file_name.ends_with(".jar") || file_name.ends_with(".jar.disabled") {
                        ...
                        if let Some(cached) = cache_dict.get(&base_name) {
                            if meta.name.is_none() { meta.name = cached.name.clone(); }
                            if meta.description.is_none() { meta.description = cached.description.clone(); }
        ...
        mods.sort_by(|a, b| b.is_enabled.cmp(&a.is_enabled).then_with(|| a.file_name.cmp(&b.file_name)));
        Ok(mods)
    }
    ...
    fn parse_single_jar(jar_path: &Path, icons_dir: &Path) -> ModMetadata {
        ...
        let mut meta = ModMetadata {
            file_name: file_name.clone(), mod_id: None, name: None, version: None, 
            description: None, icon_absolute_path: None, network_icon_url: None, 
            file_size, is_enabled: true, 
            modified_at, // ✅ 赋值给结构体
        };
            ...
            if let Ok(mut archive) = zip::ZipArchive::new(file) {
                ...
                if let Ok(mut mod_json) = archive.by_name("fabric.mod.json") {
                    ...
                    if mod_json.read_to_string(&mut contents).is_ok() {
                        if let Ok(json) = serde_json::from_str::<Value>(&contents) {
                            ...
                            if let Some(icon_path) = json["icon"].as_str() { icon_to_extract = Some(icon_path.to_string()); }
                        }
                    }
                }
                ...
                if let Some(icon_path) = icon_to_extract {
                    if let Ok(mut icon_file) = archive.by_name(&icon_path) {
                        let ext = Path::new(&icon_path).extension().unwrap_or_default().to_string_lossy();
                        let target_icon = icons_dir.join(format!("{}.{}", meta.mod_id.as_ref().unwrap_or(&file_name), ext));
                        if let Ok(mut out_file) = File::create(&target_icon) {
                            std::io::copy(&mut icon_file, &mut out_file).ok();
                            meta.icon_absolute_path = Some(target_icon.to_string_lossy().to_string());
                        }
                    }
                }
            }
    ...
    pub fn update_mod_cache<R: Runtime>(app: &AppHandle<R>, instance_id: &str, file_name: &str, name: &str, desc: &str, icon_url: &str) -> Result<(), String> {
        let instance_dir = Self::get_instance_dir(app, instance_id)?;
        ...
        cache.insert(file_name.to_string(), ModCacheInfo {
    ...
    // 保持你原有的快照功能不变...
    pub fn create_snapshot<R: Runtime>(app: &AppHandle<R>, instance_id: &str, desc: &str) -> Result<ModSnapshot, String> { /* ... */ Ok(ModSnapshot { id: "".into(), timestamp: "".into(), mod_count: 0, description: "".into() }) }
    pub fn rollback_snapshot<R: Runtime>(app: &AppHandle<R>, instance_id: &str, snapshot_id: &str) -> Result<(), String> { /* ... */ Ok(()) }
}