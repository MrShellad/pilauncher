...
export const OreBackground: React.FC = () => {
  ...
  const performanceProfile = general.performanceProfile;
  const isBatterySaver = performanceProfile === 'batterySaver';
  const isBalanced = performanceProfile === 'balanced';
  const effectiveBackgroundBlur = isBatterySaver ? 0 : appearance.backgroundBlur;
  const backgroundFilter =
    effectiveBackgroundBlur > 0 ? `blur(${effectiveBackgroundBlur}px)` : undefined;
  const panoramaPixelRatio = isBalanced ? BALANCED_PIXEL_RATIO : MAX_PIXEL_RATIO;
  const panoramaFrameInterval = isBalanced ? BALANCED_FRAME_INTERVAL_MS : 0;
  const canUsePanoramaFeature =
    appearance.panoramaEnabled && hasMicrosoftAccount && !isBatterySaver;
  const shouldPauseRotation =
    !windowActive || gameState === 'launching' || gameState === 'running';
  const shouldSuspendPanorama = isBalanced && shouldPauseRotation;
  ...
  useEffect(() => {
    ...
    if (!shouldCreatePanorama || !panoramaUrls || !container) return;

    let renderer: THREE.WebGLRenderer;
    try {
      renderer = new THREE.WebGLRenderer({
        antialias: !isBalanced,
        alpha: false,
        powerPreference: isBalanced ? 'low-power' : 'high-performance',
      });
    } catch (error) {
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, panoramaPixelRatio));
    ...
    const animate = (now: number) => {
      ...
      if (panoramaFrameInterval > 0 && now - lastFrameTime < panoramaFrameInterval) {
        return;
      }
  ...
  }, [isBalanced, panoramaFrameInterval, panoramaPixelRatio, shouldCreatePanorama, panoramaUrls]);

  const shouldMountPanorama = shouldCreatePanorama && !!panoramaUrls;
