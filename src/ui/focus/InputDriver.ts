...
const INPUT_IDLE_STOP_MS = 500;
const SCROLL_AXIS_DEADZONE = 0.15;
const SCROLL_AXIS_SPEED = 15;
...
export const useInputDriver = (
  ...
  const nativeScrollAxesRef = useRef<Record<string | number, number>>({});
  const lastAxisActionRef = useRef<Record<string | number, string>>({});
  const lastInputActivityRef = useRef(0);
  ...
  useEffect(() => {
    const hasActiveAxisInput = () =>
      Object.entries(bindings.gamepad.axes).some(([id]) => {
        const value = nativeAxesRef.current[id] ?? 0;
        return Math.abs(value) > AXIS_DEADZONE;
      });

    const getScrollAxisValue = () =>
      Object.values(nativeScrollAxesRef.current).reduce(
        (sum, value) => (Math.abs(value) > SCROLL_AXIS_DEADZONE ? sum + value : sum),
        0,
      );
    ...
    loop = (now: number) => {
      ...
      const hasActiveInput =
        currentGamepadActions.size > 0 ||
        currentKeyboardActions.size > 0 ||
        currentMouseActions.size > 0 ||
      ...
      if (hasActiveInput || now - lastInputActivityRef.current <= INPUT_IDLE_STOP_MS) {
        scheduleLoop();
      }
    };
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      lastInputActivityRef.current = performance.now();
      scheduleLoop();
    };
    ...
    listen<{
      ...
      if (
        ...
        if (isRightStickYAxis(name)) {
          nativeScrollAxesRef.current[name] = axis_value;
        }
        if (typeof axis_code === 'number' && isRightStickYAxis(code)) {
          nativeScrollAxesRef.current[code] = axis_value;
        }
        ...
        if (
          Math.abs(axis_value) > SCROLL_AXIS_DEADZONE ||
          Math.abs(axis_value) > AXIS_DEADZONE
        ) {
          lastInputActivityRef.current = performance.now();
        }
        scheduleLoop();
