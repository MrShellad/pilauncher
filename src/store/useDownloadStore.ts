...
interface DownloadStore {
  ...
  setAutoOpenOnce: (enabled: boolean) => void;
  ignoredTasks: Set<string>;
  hasAutoOpenedInCurrentBatch: boolean;
...
interface DownloadManagerPreferences {
  autoOpenOnce: boolean;
}

const DOWNLOAD_MANAGER_PREFERENCES_KEY = 'pilauncher-download-manager-preferences';

const defaultDownloadManagerPreferences: DownloadManagerPreferences = {
  autoOpenOnce: true,
};

const readDownloadManagerPreferences = (): DownloadManagerPreferences => {
  if (typeof window === 'undefined') return defaultDownloadManagerPreferences;

  try {
    const raw = window.localStorage.getItem(DOWNLOAD_MANAGER_PREFERENCES_KEY);
    if (!raw) return defaultDownloadManagerPreferences;
    return {
      ...defaultDownloadManagerPreferences,
      ...JSON.parse(raw),
    };
  } catch {
    return defaultDownloadManagerPreferences;
  }
};

const writeDownloadManagerPreferences = (preferences: DownloadManagerPreferences) => {
  if (typeof window === 'undefined') return;

  try {
    window.localStorage.setItem(DOWNLOAD_MANAGER_PREFERENCES_KEY, JSON.stringify(preferences));
  } catch {
    // Preference persistence is best effort; download state should not depend on storage.
  }
};
...
const hasAnyTasks = (tasks: Record<string, DownloadTask>) => Object.keys(tasks).length > 0;

const downloadManagerPreferences = readDownloadManagerPreferences();

export const useDownloadStore = create<DownloadStore>((set) => ({
  tasks: {},
  ignoredTasks: new Set(),
  isPopupOpen: false,
  autoOpenOnce: downloadManagerPreferences.autoOpenOnce,
  hasAutoOpenedInCurrentBatch: false,
  setPopupOpen: (isOpen) => set({ isPopupOpen: isOpen }),
  setAutoOpenOnce: (enabled) =>
    set(() => {
      writeDownloadManagerPreferences({
        autoOpenOnce: enabled,
      });
      return { autoOpenOnce: enabled };
    }),
  ...
  cancelTask: (id) =>
    set((state) => {
      ...
      return {
        ...
        hasAutoOpenedInCurrentBatch: hasAnyTasks(newTasks) ? state.hasAutoOpenedInCurrentBatch : false,
      };
    }),
  ...
  clearCompletedTasks: () =>
    set((state) => {
      const newTasks = Object.fromEntries(
        Object.entries(state.tasks).filter(([, task]) => task.status !== 'completed')
      );
