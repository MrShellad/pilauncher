...
export const ModVersionHistory: React.FC<ModVersionHistoryProps> = ({
  ...
  return (
    <div className="flex h-full w-full flex-col font-minecraft p-4 sm:p-5 bg-[var(--ore-modal-bg)]">
      {/* 顶部操作与平台切换栏 */}
      <div
        className="flex shrink-0 items-center justify-between gap-3 border-[2px] border-[#1E1E1F] bg-[#48494A] p-3 mb-3"
      ...
      >
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-white ore-text-shadow">
          <History size={15} className="text-[#6CC349]" />
          <span>可用版本列表</span>
      ...
      {/* 缺失 API Key 警告 */}
      {isCfKeyMissing && (
        <div className="mb-3 flex items-center gap-2 border-[2px] border-[#1E1E1F] bg-[#C33636] p-3 text-xs font-bold text-white shadow-[inset_0_-2px_0_#AD1D1D]">
      ...
      {isLoadingVersions ? (
        ...
        <div
          className="flex flex-1 flex-col items-center justify-center border-[2px] border-[#1E1E1F] bg-[#222324] py-12 text-center text-[#B1B2B5]"
        ...
        <div
          className="flex-1 min-h-0 border-[2px] border-[#1E1E1F] bg-[#222324] overflow-hidden"
          style={{ boxShadow: 'inset 0 2px 4px rgba(0, 0, 0, 0.4)' }}
        >
              ...
              {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                ...
                return (
                  <div
                    ...
                    className={`grid h-[74px] max-h-[74px] min-h-[74px] grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b-[2px] border-[#1E1E1F] px-4 transition-none ${
                      isCurrent
                        ? 'bg-[#3C8527] text-white shadow-[inset_0_2px_0_rgba(255,255,255,0.25),inset_0_-3px_0_#1D4D13]'
                        : 'bg-[#48494A] text-white shadow-[inset_0_2px_0_rgba(255,255,255,0.12),inset_0_-2px_0_rgba(0,0,0,0.35)] hover:bg-[#58595B]'
                    }`}
                  >
                    {/* 左侧：版本名 + 各种标签 + 发布时间 */}
                    <div className="flex flex-col justify-center min-w-0 pr-2">
                      <div className="flex items-center gap-2 min-w-0">
                        {/* 当前版本指示 */}
                        {isCurrent && (
                          <span className="flex shrink-0 items-center gap-1 border-[2px] border-[#1E1E1F] bg-[#1D4D13] px-1.5 py-0.5 text-[10px] font-bold uppercase text-[#6CC349]">
                      ...
                      {/* 下行：游戏版本 + Loader + 发布日期 */}
                      <div className="mt-1 flex items-center gap-2 text-[11px] text-[#D0D1D4]">
