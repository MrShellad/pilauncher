...
const renderEnvChip = (env: string | undefined, type: 'client' | 'server') => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 border-[2px] px-1.5 py-0.5 font-minecraft text-[10px] font-bold uppercase tracking-wider ${
        isRequired
...
export const ModHeader: React.FC<ModHeaderProps> = ({ mod, displayMod, instanceId, onClose }) => {
  ...
  return (
    ...
    >
      {/* 左侧：48px 模组图标 + 标题与附属元数据 */}
      <div className="flex items-center gap-3.5 min-w-0 flex-1">
        {/* 48px 图标 */}
        <div
          className="relative flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden border-[2px] border-[#1E1E1F] bg-[#313233]"
          style={{ boxShadow: 'inset 0 -2px 0 rgba(0, 0, 0, 0.4), inset 2px 2px 0 rgba(255, 255, 255, 0.12)' }}
        >
          {mod.isFetchingNetwork && (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/60">
              <Loader2 className="animate-spin text-[#6CC349]" size={16} />
            </div>
          )}
          {detailIconUrl ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-[#2B2C2D]">
              <Blocks size={24} className="text-[#8C8D90]" />
            </div>
          )}
        </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {networkInfo && (
              <span className="text-xs text-[#B1B2B5]">
