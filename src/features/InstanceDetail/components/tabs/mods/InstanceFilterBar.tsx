    ...
    <div className="p-4 bg-[#18181B] border-2 border-[#2A2A2C] flex-shrink-0 z-20 mb-4 shadow-md">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <OreButton focusKey="inst-filter-back" variant="ghost" size="auto" onClick={props.onBack} className="!h-[44px] !px-4 text-gray-400 hover:text-white border border-white/5 bg-black/30">
          ...
          <div className="w-[280px] relative focus-within:z-50">
            <FocusItem focusKey="inst-filter-source" onEnter={() => props.setSource(props.source === 'modrinth' ? 'curseforge' : 'modrinth')}>
              {({ ref, focused }) => (
          ...
          <div className="flex-1 min-w-[140px] relative focus-within:z-50">
            <OreDropdown focusKey="inst-filter-category" options={categoryOptions} value={props.category || categoryOptions[0].value} onChange={props.setCategory} className="w-full !h-[44px]" />
          </div>
          
          <div className="flex-1 min-w-[140px] relative focus-within:z-50">
            <OreDropdown focusKey="inst-filter-sort" options={sortOptions} value={props.sort || sortOptions[0].value} onChange={props.setSort} className="w-full !h-[44px]" />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="w-[120px] relative focus-within:z-50">
            <OreButton focusKey="inst-filter-btn-search" variant="primary" size="auto" onClick={props.onSearch} className="w-full !h-[44px] font-bold text-black tracking-wider">
              <Search size={16} className="mr-1.5" /> 搜索
            </OreButton>
          </div>
          <div className="w-[100px] relative focus-within:z-50">
            <OreButton focusKey="inst-filter-btn-reset" variant="secondary" size="auto" onClick={props.onReset} className="w-full !h-[44px] text-black">
