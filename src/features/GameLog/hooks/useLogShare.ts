...
export const useLogShare = () => {
  ...
  const shareLogs = useCallback(async (logs: string[], options: LogShareOptions) => {
    ...
    if (!content) {
      const message = t('gameLog.share.noLogs', '暂无可上传的日志内容');
      setError(message);
    ...
    try {
      ...
      addToast('success', t('gameLog.share.success', '日志已上传到 LogShare.CN'));
      return nextReport;
    } catch (shareError) {
  ...
  const copyShareUrl = useCallback(async () => {
    ...
    addToast('success', t('gameLog.share.copied', '分享链接已复制'));
    window.setTimeout(() => setCopiedShareUrl(false), 2000);
  }, [addToast, report?.upload.url, t]);
