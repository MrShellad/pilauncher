...
// Use a hook-aware wrapper inside components instead of a global const renderIssueText,
// or we can pass t into renderIssueText. Let's pass t.
const renderIssueText = (issue: unknown, index: number, t: any) => {
  if (typeof issue === 'string') return issue;
  if (!isRecord(issue)) return t('gameLog.shareDialog.issueFallback', '问题 {{index}}', { index: index + 1 });
  return getString(issue, ['message', 'title', 'name', 'description', 'summary']) ?? t('gameLog.shareDialog.issueFallback', '问题 {{index}}', { index: index + 1 });
};
...
export const LogShareDialog: React.FC<LogShareDialogProps> = ({
  ...
  return (
    <OreModal
      isOpen={isOpen}
      onClose={onClose}
      title={t('gameLog.shareDialog.title', '日志分享')}
      className="w-[620px]"
        ...
        report ? (
          <>
            ...
            >
              ...
              {t('gameLog.shareDialog.openPage', '打开页面')}
            </OreButton>
      ...
      <div className="space-y-4">
        <div className="flex items-start gap-3 rounded-sm border border-sky-500/20 bg-sky-500/10 p-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 text-xs leading-relaxed text-ore-text-muted">
              {t('gameLog.shareDialog.uploadInfo', '当前准备上传 {{count}} 行日志。上传前会在后端执行大小校验和可选脱敏。', { count: logCount })}
            </div>
          </div>
        </div>
        ...
        {!report && (
          <div className="space-y-3">
            <ToggleRow
              ...
              checked={sanitize}
              title={t('gameLog.shareDialog.sanitizeTitle', '脱敏敏感信息')}
              description={t('gameLog.shareDialog.sanitizeDesc', '替换常见 token、授权头和本机用户路径。')}
              disabled={isSharing}
            ...
            <ToggleRow
              ...
              title={t('gameLog.shareDialog.aiTitle', '请求 AI 诊断')}
              description={t('gameLog.shareDialog.aiDesc', '上传成功后附加拉取智能诊断，可能比普通分享更慢。')}
              disabled={isSharing}
        ...
        {isSharing && (
          <div className="flex items-center rounded-sm border border-white/10 bg-white/[0.03] p-3 text-sm text-ore-text-muted">
            ...
            {t('gameLog.shareDialog.analyzing', '正在上传并获取分析结果...')}
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="rounded-sm border border-ore-green/25 bg-ore-green/10 p-3">
              <div className="flex items-center gap-2 text-sm text-ore-green">
                <ShieldCheck size={16} />
                {t('gameLog.shareDialog.uploadSuccess', '上传成功')}
              </div>
              ...
              <div className="mt-2 text-xs text-ore-text-muted">
                {report.lineCount} 行 / {formatBytes(report.byteCount)} / {report.sanitized ? t('gameLog.shareDialog.sanitized', '已脱敏') : t('gameLog.shareDialog.unsanitized', '未脱敏')}
              </div>
              {report.history && (
                <div className="mt-2 text-xs text-ore-green">
                  {t('gameLog.shareDialog.historyInfo', '已记录远端日志删除凭据，过期时间会用于后续数据管理。')}
                </div>
              )}
              {report.historyError && (
                <div className="mt-2 text-xs text-yellow-200">
                  {t('gameLog.shareDialog.historyError', '历史记录保存失败：{{error}}', { error: report.historyError })}
                </div>
              )}
            </div>
            ...
            {(software || version || issues.length > 0 || report.insightsError) && (
              <div className="rounded-sm border border-white/10 bg-white/[0.03] p-3">
                <div className="mb-2 text-sm text-white">{t('gameLog.shareDialog.insights', '基础洞察')}</div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <div className="text-ore-text-muted">{t('gameLog.shareDialog.softwareType', '软件类型')}</div>
                    <div className="mt-1 text-white">{software ?? t('gameLog.shareDialog.unidentified', '未识别')}</div>
                  </div>
                  <div>
                    <div className="text-ore-text-muted">{t('gameLog.shareDialog.version', '版本')}</div>
                    <div className="mt-1 text-white">{version ?? t('gameLog.shareDialog.unidentified', '未识别')}</div>
                  </div>
                </div>
                {issues.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {issues.slice(0, 5).map((issue, index) => (
                      <div key={index} className="rounded-sm border border-yellow-500/20 bg-yellow-500/10 px-3 py-2 text-xs text-yellow-100">
                        {renderIssueText(issue, index, t)}
                      </div>
                    ))}
                  </div>
                )}
                {report.insightsError && (
                  <div className="mt-3 text-xs text-yellow-200">{t('gameLog.shareDialog.insightsError', '洞察获取失败：{{error}}', { error: report.insightsError })}</div>
                )}
              </div>
            )}
            ...
            {(aiText || report.aiAnalysisError) && (
              <div className="rounded-sm border border-purple-400/20 bg-purple-400/10 p-3">
                <div className="mb-2 flex items-center gap-2 text-sm text-white">
                  ...
                  {t('gameLog.shareDialog.aiDiagnosis', 'AI 诊断')}
                </div>
                {aiText && (
                ...
                {report.aiAnalysisError && (
                  <div className="text-xs text-yellow-200">{t('gameLog.shareDialog.aiAnalysisError', 'AI 诊断获取失败：{{error}}', { error: report.aiAnalysisError })}</div>
                )}
              </div>
            )}
          </div>
