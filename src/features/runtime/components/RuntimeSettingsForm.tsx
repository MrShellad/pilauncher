...
import { TestTube2, Loader2, RotateCcw } from 'lucide-react';
...
export const RuntimeSettingsForm: React.FC<{
  ...
  const focusOrder = useMemo(() => {
    ...
    if (!(isInstance && config.useGlobalMemory)) {
      order.push('java-memory-mode', 'java-slider-memory', 'java-btn-recommend', 'java-input-jvm', 'java-btn-reset-jvm');
    }
              ...
              control={
                <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center">
                  ...
                  <OreButton
                    ...
                    onArrowPress={handleLinearArrow('java-btn-reset-jvm')}
                    size="auto"
                    variant="secondary"
                    onClick={() => handleChange('jvmArgs', '')}
                    disabled={(isInstance && config.useGlobalMemory) || !config.jvmArgs}
