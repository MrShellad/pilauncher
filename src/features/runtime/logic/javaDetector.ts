...
export const deleteManagedJava = async (javaPath: string): Promise<JavaInstall[]> => {
  return invoke<JavaInstall[]>('delete_managed_java', { javaPath });
};

// 智能判断适用版本
export const getJavaRecommendation = (versionStr: string): string => {
  if (versionStr.includes('25.')) return '推荐 26.1+ / 新快照';
