...
export const AccountCard: React.FC<AccountCardProps> = ({
  ...
  const isAuthlib = rawType.toLowerCase() === 'authlib';
  const isOffline = !isMS && !isAuthlib;
  const displayName = account.name || (account as any).username || t('settings.account.card.unknownPlayer');
  ...
  const cardTypeClass = isMS ? 'is-premium' : isAuthlib ? 'is-authlib' : 'is-offline';
  ...
  return (
    <div className={`ore-account-card ${cardTypeClass} ${isActive ? t('settings.account.card.current') : t('settings.account.card.switchable')}`}>
      <div className="ore-account-head">
        <div className="ore-account-tags">
          <span className={`ore-account-type-tag ${cardTypeClass}`}>
            {isMS ? t('settings.account.card.typeMicrosoft') : isAuthlib ? t('settings.account.card.typeAuthlib') : t('settings.account.card.typeOffline')}
          </span>
          <span className={`ore-account-status-tag ${isActive ? t('settings.account.card.current') : t('settings.account.card.switchable')}`}>
            {isActive ? t('settings.account.card.current') : t('settings.account.card.switchable')}
          </span>
        </div>
      </div>
      ...
      <div className="ore-account-hero">
        <div className="ore-card-avatar-wrapper">
          ...
          <div className={`ore-account-avatar-emblem ${cardTypeClass}`}>
            {isMS ? <ShieldCheck size={14} /> : isAuthlib ? <Server size={14} /> : <UserRound size={14} />}
          </div>
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="ore-account-subtitle" title={displayUuid}>
            {isMS ? 'Minecraft Java Edition' : isAuthlib ? (account.authlibApiRoot || 'Authlib Injector') : `${displayUuid}`}
          </div>
          <div className="ore-account-note">
            {isMS ? t('settings.account.card.noteMicrosoft') : isAuthlib ? t('settings.account.card.noteAuthlib') : t('settings.account.card.noteOffline')}
          </div>
        </div>
      </div>
      ...
      <div className="ore-account-toolbar justify-center">
        {isOffline && (
          <>
            {renderIconButton(
              ...
              t('settings.account.card.uploadSkin'),
              <ImagePlus size={18} />,
              'accent',
            ...
            {renderIconButton(
              ...
              t('settings.account.card.editName'),
              <Pencil size={18} />,
              'neutral',
              () => onEdit(account)
            )}
        ...
        {renderIconButton(
          ...
          t('settings.account.card.removeAccount'),
          <Trash2 size={18} />,
          'danger',
        ...
        {isActive ? (
          ...
          <FocusItem focusKey={`btn-active-${account.uuid}`} onEnter={() => onSetCurrent(account.uuid)}>
            {({ ref, focused }) => (
              ...
              >
                {t('settings.account.card.setCurrent')}
              </button>
            )}
          </FocusItem>
        )}
