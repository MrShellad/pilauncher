              ...
              {report.domains.map((domainResult) => (
                ...
                >
                  {({ ref, focused }) => (
                    <div
                      ref={ref}
                      className={`flex flex-col justify-between rounded-lg border bg-black/20 p-4 transition-colors hover:bg-black/30 ${
                        focused
              ...
              {report.qrcode_uri && (
                <FocusItem focusKey={NETWORK_DIAGNOSTICS_QR_FOCUS_KEY} onArrowPress={onArrowPress}>
                  {({ ref, focused }) => (
                    <div
                      ref={ref}
                      className={`flex w-full shrink-0 flex-col items-center justify-center rounded-lg border p-4 transition-all lg:w-48 ${
                        focused ? 'border-ore-green bg-white/10' : 'border-white/10 bg-white/5'
                      }`}
                    >
