...
export function useWardrobeSession() {
  ...
  const touchAccountSkinCache = useCallback(
    (
      ...
      skinUrl?: string | null,
      capeUrl?: string | null,
      model?: ReturnType<typeof resolveSkinModel>
    ) => {
      const nextUrl = (skinUrl || account.skinUrl || account.uuid).split('?')[0];
      const currentUrlNoTs = account.skinUrl?.split('?t=')[0];
      const nextCapeUrl = capeUrl?.split('?')[0] || null;
      const currentCapeUrlNoTs = account.capeUrl?.split('?t=')[0] || null;
      const updates: Partial<MinecraftAccount> = {};

      // 如果 URL 没变（忽略时间戳），则不更新 Store 以免引起重绘
      if (currentUrlNoTs !== nextUrl) {
        updates.skinUrl = `${nextUrl}?t=${Date.now()}`;
      }

      if (currentCapeUrlNoTs !== nextCapeUrl) {
        updates.capeUrl = nextCapeUrl ? `${nextCapeUrl}?t=${Date.now()}` : null;
      }

      if (model && account.metadata?.model !== model) {
        updates.metadata = {
          ...(account.metadata ?? {}),
          model,
        };
      ...
      updateAccount(account.uuid, updates);
    },
    [updateAccount]
  );
      ...
      try {
        if (isMicrosoft) {
          ...
          if (activeLocalAsset?.variant) {
            const nextModel = resolveSkinModel(activeLocalAsset.variant);
            onModelResolved(nextModel);
            touchAccountSkinCache(account, activeLocalAsset.filePath, null, nextModel);
          }
