...
export const getLocalizedDownloadTagLabel = ({
  ...
  const translationKeys = Array.from(
    new Set(
      [
        translationKey,
        raw ? getDownloadCategoryTranslationKey(raw) : '',
        display ? getDownloadCategoryTranslationKey(display) : ''
      ].filter((key): key is string => Boolean(key))
    )
  );

  for (const candidateKey of translationKeys) {
    const translatedLabel = t(candidateKey, { defaultValue: '' });
    if (typeof translatedLabel === 'string' && translatedLabel.trim().length > 0 && translatedLabel !== candidateKey) {
