...
export const InstanceSelectModal: React.FC<InstanceSelectModalProps> = ({ isOpen, version, onClose, onConfirm }) => {
  ...
  useEffect(() => {
    if (isOpen && version) {
      setIsLoading(true);
      // ✅ 调用新的后端精准过滤命令 (注意这里利用了 Tauri 自动将驼峰转为蛇形的特性)
      invoke<any[]>('get_compatible_instances', {
        ...
        .then(list => {
          ...
          if (list && list.length > 0) {
            setSelectedId(list[0].id); // 默认选中第一项
          }
        })
        .catch(console.error)
    ...
    <OreModal isOpen={isOpen} onClose={onClose} hideTitleBar={false} className="w-full max-w-lg bg-[#18181B] p-0">
      <FocusBoundary id="instance-select-boundary" className="flex flex-col max-h-[70vh]">
        ...
        <div className="p-5 border-b border-white/5 bg-black/40 text-sm text-gray-300">
          ...
          <div className="text-xs text-gray-500 mt-1">兼容要求：{version.game_versions[0]} | {version.loaders.join(', ')}</div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar min-h-[250px]">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-500">
              ...
              <div className="text-sm">没有找到符合当前版本和引导器的本地实例</div>
              <div className="text-xs mt-2 opacity-60">请先在启动器主页创建一个兼容的游戏实例</div>
            </div>
          ) : (
            instances.map(inst => {
                ...
                <FocusItem key={inst.id} onEnter={() => setSelectedId(inst.id)}>
                  {({ ref, focused }) => (
                    <div
                      ...
                      className={`
                        flex items-center p-3 rounded-sm border transition-all cursor-pointer
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        {/* 我们后端返回的字段是 inst.version 和 inst.loader，现在能精准对上了 */}
        ...
        <div className="p-4 border-t border-white/5 bg-black/60 flex justify-end gap-3 flex-shrink-0">
          <OreButton variant="secondary" onClick={onClose}>取消</OreButton>
          <OreButton variant="primary" disabled={!selectedId || instances.length === 0} onClick={() => onConfirm(selectedId)}>
            确认下载
          </OreButton>
        </div>
