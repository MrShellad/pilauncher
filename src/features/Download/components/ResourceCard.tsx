...
// ── focus 系统：上箭头从 grid 逃逸到 filter bar 的候选顺序 ──────────────────
...
// ── 工具：时间格式化（保留在此以避免 i18n context 依赖） ────────────────────
function useTimeAgo() {
...
export const ResourceCard = React.memo(({
  ...
  const { loaders, features, followerCount, supportsClient, supportsServer } = vm;
        ...
        return (
          ...
          >
            ...
            <div className="flex w-full gap-[0.875rem] p-[0.9375rem] pr-[1rem]">
              ...
              {/* 右列：标题行（含徽标）+ 描述 + 分类标签 + 统计栏 */}
              <div className="flex min-w-0 flex-1 flex-col justify-between">
                <div className="flex min-h-[8rem] flex-col justify-between">
                  <div className="min-h-[4.25rem]">
                    {/* 标题行：徽标 shrink-0 排在右侧，标题 min-w-0 截断，二者共享同一行 */}
